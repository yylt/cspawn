...
test_local() {
    ...
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" /bin/echo "Hello from cspawn"
    
    # Test 2: Single environment variable
    echo "Test 2: Single environment variable"
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -e TEST_VAR=hello123 /bin/sh -c 'echo $TEST_VAR' | grep -q "hello123"
    
    # Test 3: Multiple environment variables
    echo "Test 3: Multiple environment variables"
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -e VAR1=value1 -e VAR2=value2 /bin/sh -c 'echo $VAR1-$VAR2' | grep -q "value1-value2"
    ...
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -E /tmp/cspawn-e2e-env.txt /bin/sh -c 'echo $ENV_FILE_VAR-$ENV_FILE_VAR2' | grep -q "test_value-test_value2"
    
    # Test 5: Working directory
    echo "Test 5: Working directory"
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -c /tmp /bin/sh -c 'pwd' | grep -q "/tmp"
    ...
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -b /tmp/cspawn-e2e-host-data:/container/data /bin/cat /container/data/test.txt | grep -q "bound data"
    
    # Test 7: Bind mount (read-only)
    echo "Test 7: Bind mount (read-only)"
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -b /tmp/cspawn-e2e-host-data:/container/ro-data:ro /bin/cat /container/ro-data/test.txt | grep -q "bound data"
...
test_user() {
    ...
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -u root:root /bin/id | grep -q "uid=0"
    
    # Test 2: Nobody user
    echo "Test 2: Nobody user"
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" -u nobody:nogroup /bin/id | grep -q "uid=65534"
...
test_combined() {
    ...
    run_sudo "$CSPAWN_BIN" -r local -d "$ROOTFS_PATH" /bin/sh -c 'echo "arg1 arg2"' | grep -q "arg1 arg2"
